...
public class MainController {
    ...
    public String start(Model model) {
        ...
        List<ApiRepository> apiRepositories = new ArrayList<>();

        for (int i = 0; i < entityList.size(); i++) {
            ApiList apiList = new ApiList();
            apiList.setApiId(entityList.get(i).getApiId());
            apiList.setApiUrl(entityList.get(i).getApiUrl());
            apiList.setServiceKey(entityList.get(i).getServiceKey());
            apiList.setApiKor(entityList.get(i).getApiKor());

            apiRepositories.add(apiList.toEntity());
        }

        model.addAttribute("apiRepository", apiRepositories);
